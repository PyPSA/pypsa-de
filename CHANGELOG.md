# Changelog
- Added `pypsa-de: daily_fuel_prices`: optionally override the constant gas supply price with real daily gas prices (THE EGSI day-ahead) of the scenario's weather year, applied to the network of a chosen planning horizon (default 2025). Reads `data/pypsa-de/daily_fuel_prices.csv`, which keeps the raw nominal `gas` column (EUR/MWh_LHV, covering 2023-2025) for provenance and a precomputed `gas_real2020` column in real 2020 EUR; `modify_prenetwork` maps the deflated column onto the snapshots as time-varying `marginal_cost` on the fossil gas supply generators. The deflation is precomputed once by `scripts/pypsa-de/deflate_daily_fuel_prices.py` (German GDP deflator via `pydeflate`, same method as `build_monthly_prices`), so no network dependency is hit at model runtime. The mechanism is generic over `fuels`, so further commodities can be added by extending the CSV and the `fuels` list.
- Bugfix: `add_electricity` now also catches `KeyError` when selecting the country-specific conventional-generator column for the snapshot year, so weather years absent from those inputs fall back to the last (most recent) available column as already intended.
- Bugfix: `build_population_weighted_energy_totals` no longer crashes for weather years beyond the heat-totals coverage (which ends 2023). When the requested year is unavailable it falls back to the nearest available year (with a warning); the hourly heat demand profile still follows the weather-year cutout while the annual magnitude uses the fallback year.
- Bugfix: `build_electricity_demand` no longer crashes when the snapshot window extends beyond the synthetic demand data (e.g. weather years 2024/2025). The synthetic supplement now fills only where it has data instead of strict-indexing the full snapshot range, so weather years covered by real ENTSO-E demand can be used. Genuine gaps are still caught by the no-NaN assert.
