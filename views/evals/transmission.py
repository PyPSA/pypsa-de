...
from esmtools.constants import BusCarrier, Carrier, DataModel
from esmtools.plots.gridmap import GridMapConfig, TransmissionGridMap
from esmtools.statistic import collect_myopic_statistics
from esmtools.utils import filter_by, make_evaluation_result_directories
...
def eval_grid_capactiy(
    ...
    for n in networks.values():
        # Lower Austria
        n.df("Bus").loc["AT0 1", "x"] -= 0.1  # Lon, left
        n.df("Bus").loc["AT0 1", "y"] += 0.3  # Lat, up
        # Burgenland
        n.df("Bus").loc["AT0 0", "x"] -= 0.1  # Lon, left
        n.df("Bus").loc["AT0 0", "y"] -= 0.26  # Lat, down
        # Salzburg
        n.df("Bus").loc["AT0 6", "x"] += 0.35  # Lon, right
        n.df("Bus").loc["AT0 6", "y"] -= 0.05  # Lat, down
        # Vienna
        # n.df("Bus").loc["AT0 2", "x"] += 0.1  # Lon, right
        n.df("Bus").loc["AT0 2", "y"] += 0.1  # Lat, up
