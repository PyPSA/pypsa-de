...
def plot_capacity_comparison(
    ...
    demand_order = base[base < 0].abs().sort_values(ascending=False).index  # sort by |capacity|
    ...
    if plot_diff:
        ...
        df_plot_supply = df[[base_name, cmp_name]].loc[supply_order.intersection(df.index)]
        df_plot_demand = df[[base_name, cmp_name]].loc[demand_order.intersection(df.index)]
    ...
    for j, col in enumerate(plot_columns):
        if n_supply:
            ...
            ax.bar(
                pos_supply + (j - 0.5) * bar_width if len(plot_columns) > 1 else pos_supply,
                vals,
                width=bar_width if len(plot_columns) > 1 else 0.8,
                color=[tech_colors.get(t, "#1f77b4") for t in df_plot_supply.index],
                hatch=("" if (not plot_diff and col == base_name) else (hatch_second if not plot_diff else "")),
                alpha=0.9,
                linewidth=0.0,
            )
    ...
    for j, col in enumerate(plot_columns):
        if n_demand:
            vals = df_plot_demand[col].values if not plot_diff else (df_plot_demand[col].values)
            # In diff mode we plot signed values; in normal mode they are already positive magnitudes
            ax.bar(
                pos_demand + (j - 0.5) * bar_width if len(plot_columns) > 1 else pos_demand,
                vals,
                width=bar_width if len(plot_columns) > 1 else 0.8,
    ...
    if plot_diff:
        ...
        handles = [
            mpatches.Patch(facecolor="white", edgecolor="black", hatch="", label=base_name),
            mpatches.Patch(facecolor="white", edgecolor="black", hatch=hatch_second, label=cmp_name),
        ]
...
if __name__ == "__main__":
    ...
    for year in planning_horizons:
        ...
        capa_comp_agg = capa_comp_agg[
            (capa_comp_agg.abs() >= 0.1).any(axis=1)
        ]
        ...
        plot_capacity_comparison(
            ...
            save_path=snakemake.output.elec_capa_comp_de_2025 if year == 2025 else snakemake.output[-1] + f"/elec_capa_comp_de_{year}.png",
        )
    ...
    for year in planning_horizons:
        for scenario in scenarios:
            ...
            capacities = (
                ...
                .loc[lambda df: ~df.index.get_level_values("bus").str.contains("DE", regex=False)]
                .groupby("carrier").sum()
    ...
    for year in planning_horizons:
        ...
        # drop capa with less than 100 MW in both scenarios
        demand_comp_agg = demand_comp_agg[
            (demand_comp_agg.abs() >= 0.1).any(axis=1)
        ]
        ...
        for scenario in scenarios:
            ...
            capex = df[df.index.get_level_values("Variable").str.startswith("System Cost|CAPEX")]
            capex_top = capex[capex.index.get_level_values("Variable").str.count(r"\|") == 2]
            ...
            if scenario == scenarios[0]:
                year_cols = [col for col in capex_reset.columns if str(col) in ['2025', '2030', '2035']]
            
            # Store processed data
            capex_data[scenario] = capex_reset.set_index('Category')[year_cols].fillna(0)
        ...
        for i, category in enumerate(categories):
            ...
            ariadne_values = [capex_ariadne_plot.loc[category, col] for col in year_cols]
            
            # Plot bars - LowDemand with hatching, AriadneDemand without
            ax.bar(x - width/2, low_values, width, bottom=bottom_low, 
                label=category, color=colors[i], alpha=0.8, hatch='///')
            ax.bar(x + width/2, ariadne_values, width, bottom=bottom_ariadne, 
            ...
            for j in range(len(years)):
                ...
                if low_values[j] > 0.5:  # Only show if value is significant
                    ax.text(j - width/2, bottom_low[j] + low_values[j]/2, 
                        f'{low_values[j]:.1f}', ha='center', va='center', 
                        fontsize=8, fontweight='bold', color='white')
                
                # AriadneDemand category values
                if ariadne_values[j] > 0.5:  # Only show if value is significant
                    ax.text(j + width/2, bottom_ariadne[j] + ariadne_values[j]/2, 
                        f'{ariadne_values[j]:.1f}', ha='center', va='center', 
                        fontsize=8, fontweight='bold', color='white')
        ...
        ax.set_title('System Cost CAPEX Comparison: LowDemand vs AriadneDemand', fontsize=14, fontweight='bold')
        ax.set_xticks(x)
        ax.set_xticklabels(years)

        # Category legend elements
        category_elements = [Rectangle((0,0),1,1, facecolor=colors[i], alpha=0.8, label=cat) 
                            for i, cat in enumerate(categories)]

        # Scenario legend elements
        scenario_elements = [
            Rectangle((0,0),1,1, facecolor='gray', alpha=0.8, hatch='///', label='LowDemand'),
            Rectangle((0,0),1,1, facecolor='gray', alpha=0.8, label='AriadneDemand')
        ]

        # Create separate legends
        category_legend = ax.legend(handles=category_elements, loc='upper left', bbox_to_anchor=(1.05, 1), title='Categories')
        scenario_legend = ax.legend(handles=scenario_elements, loc='upper left', bbox_to_anchor=(1.05, 0.6), title='Scenarios')
        ...
        for i, year in enumerate(years):
            ax.text(i - width/2, bottom_low[i] + max(max(bottom_low), max(bottom_ariadne)) * 0.02, 
                    f'{bottom_low[i]:.1f}', ha='center', fontweight='bold', fontsize=9)
            ax.text(i + width/2, bottom_ariadne[i] + max(max(bottom_low), max(bottom_ariadne)) * 0.02, 
                    f'{bottom_ariadne[i]:.1f}', ha='center', fontweight='bold', fontsize=9)
        ...
        plt.savefig(snakemake.output[-1] + f"/capex_de.png", bbox_inches="tight")
        plt.close()
