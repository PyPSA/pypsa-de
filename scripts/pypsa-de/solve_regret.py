...
def fix_capacities_strict(realization, decision):
    n = realization.copy()
...
def fix_capacities(realization, decision):
    ...
    for name, attr in nominal_attrs.items():
        ...
        if name == "links":
            ...
            _idx = new.loc[new.carrier.isin(virtual_links)].index.intersection(
                extendable_i
            )
...
if __name__ == "__main__":
    ...
    n = fix_capacities_strict(realization, decision)
    ...
    #     p_nom=1e6,
    #     marginal_cost=1000, # marginal cost in €/tCO2
    ...
    results = {}
    crazy = {}
    for addon in np.logspace(0, 3, 10):
        m = n.copy()
        m.global_constraints.loc[["CO2Limit"], "constant"] += addon
        ...
        prepare_network(
            ...
            regret_run=snakemake.params.get("regret_run", False),
        )
        with memory_logger(
        ...
        results[addon] = m.global_constraints.loc["CO2Limit", "mu"]
        crazy[addon] = (
            decision.global_constraints.loc["CO2Limit", "constant"]
            - m.global_constraints.loc["CO2Limit", "constant"]
        )

    n.global_constraints.loc[["CO2Limit"], "constant"] += 1000
    # n.global_constraints.loc[["co2_sequestration_limit"], "constant"] -= 100
