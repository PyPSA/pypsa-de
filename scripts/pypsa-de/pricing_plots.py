...
def plot_supply_demand(
    ...
    demand = demand[
        demand[d] > 1
    ]  # Drop all technologies with demand less than 1e-1 MW
    demand.loc[:, d] = demand[d] / 1e3  # Convert to GW
    demand.loc[:, "bidding_price"] = demand.bidding_price.clip(
        lower=0
    )  # Clip negative bidding prices to 0
    ...
    max_finite_bidding_price = demand.bidding_price[
        demand.bidding_price.apply(np.isfinite)
    ].max()
    demand["bidding_price"] = demand["bidding_price"].replace(
        np.inf, max_finite_bidding_price * 1.1
    )
    ...
    for index, row in supply.iterrows():
        ...
        ax.add_patch(
            plt.Rectangle(
                ...
                width,
                height,
                color=tech_colors[str(row["carrier"])],
                alpha=0.5,
                label=carrier_label
                if carrier_label not in supply_labels
                else "_nolegend_",
            )
        )
    ...
    if demand_plot:
        ...
        for i in range(len(demand) - 1):
            ...
            if demand_text & (i < len(demand) - 2):
                ...
                demand_legend[number_label] = (
                    demand.index[i + 2] if not only_carriers else demand.carrier[i + 1]
                )
            ax.plot(
                [x1, x1],
                [demand.iloc[i]["bidding_price"], y2],
                color="grey",
                linestyle="--",
            )
    ...
    supply_handles_labels.pop(
        "_nolegend_", None
    )  # Remove the dummy label used to avoid duplicates

    # Create demand legend handles and labels
    demand_legend_handles = [
        plt.Line2D([0], [0], color="white", label=f"{num}: {label}")
        for num, label in demand_legend.items()
    ]

    # Plot market clearing point if there is only one bus
    if len(buses) == 1:
        ax.plot(
            n.statistics.supply(bus_carrier="AC", aggregate_time=False)[timestep].sum()
            / 1e3,
            n.buses_t.marginal_price.loc[timestep, buses],
            marker="x",
            markersize=7,
            color="red",
    ...
    legend1 = plt.legend(
        handles=supply_handles_labels.values(),
        labels=supply_handles_labels.keys(),
        title="Supply",
        loc="upper left",
...
if __name__ == "__main__":
    ...
    colors_update = (
        networks[2020]
        .carriers.color.rename(networks[2020].carriers.nice_name)
        .to_dict()
    )
    ...
    for year in planning_horizons:
        ...
        for i in range(num_subplots):
            ...
            supply_handles_labels = plot_supply_demand_s(
                n,
                supply,
                demand,
                buses,
        ...
        fig.legend(
            handles=all_supply_handles_labels.values(),
        ...
        plt.savefig(
            f"{snakemake.output.merit_order_3cases}/{year}.png", bbox_inches="tight"
        )
        plt.close(fig)
    ...
    for year, ylim in zip(
        planning_horizons,
        [(-10, 100), (-10, 150), (-10, 200), (-10, 300), (-10, 400), (-10, 500)],
    ):
        ...
        plt.savefig(
            f"{snakemake.output.price_setter}/{year}-price-setter.png",
            bbox_inches="tight",
        )
    ...
    for year, ylim in zip(
        ...
        plt.legend(bbox_to_anchor=(1, 1))
        plt.savefig(
            f"{snakemake.output.price_taker}/{year}-price-taker.png",
            bbox_inches="tight",
        )
    ...
    for year in planning_horizons:
        ...
        for carrier in df.carrier.unique():
            plt.plot(
                df[df.carrier == carrier]["marginal price @ bus"],
                marker="|",
                linestyle="",
                label=carrier,
                color=tech_colors[carrier],
        ...
        plt.savefig(
            f"{snakemake.output.pdc_price_setter}/{year}-pdc-price-setter.png",
            bbox_inches="tight",
        )
    ...
    for year in years:
        ...
        plt.ylim(-10, 200)
        plt.title(
            f"Price duration curve for electricity by highest price taker in {year}"
        )
        plt.xlabel("Hours of the year in 3h steps (sorted)")
        plt.ylabel("Market clearing price in EUR/MWh)")
        plt.legend(bbox_to_anchor=(1, 1), fancybox=True, shadow=True, ncol=1)
        plt.savefig(
            f"{snakemake.output.pdc_price_taker}/{year}-pdc-price-taker.png",
            bbox_inches="tight",
        )
    ...
    for i, n in enumerate(networks.values()):
        ...
        ax.set_title("Electricity price duration curves", fontsize=16)
        ax.legend()
        ax.grid(True)
    ...
    df_s_grouped = pd.concat(
        [
            df_s_grouped,
            df_res_absolut.loc[:, ~df_res_absolut.columns.isin(carrier_groups.keys())],
        ],
        axis=1,
    )
    ...
    df_d_grouped = pd.concat(
        [
            df_d_grouped,
            df_res_absolut.loc[
                :, ~df_res_absolut.columns.isin(carrier_groups_d.keys())
            ],
        ],
        axis=1,
    )
    ...
    res = pd.DataFrame(
        ...
        columns=[
            "Only supply price setter",
            "Only demand price setter",
            "Supply and demand price setter",
        ],
    )
    for year in planning_horizons:
    ...
    color_list = [
        group_colors.get(carrier, tech_colors.get(carrier))
        for carrier in df_all_grouped.columns
    ]
    df_all_grouped.plot(
        kind="area", stacked=True, figsize=(8, 6), color=color_list, ax=ax
    )
