...
class _TransmissionLimitCountriesConfig(BaseModel):
    """Configuration for `electricity.transmission_limit_countries` settings."""

    enable: bool = Field(
        False,
        description="Apply separate transmission expansion limits to lines/links internal to `country` and to its cross-border interconnectors, instead of a single limit for the whole model. The global `transmission_limit` continues to apply to all remaining lines/links (those with neither bus in `country`).",
    )
    country: str = Field(
        "DE",
        description="ISO2 country code for which internal grid and interconnector expansion are limited separately.",
    )
    internal: str = Field(
        "v1.0",
        description="Transmission expansion limit for lines/links with both buses in `country`. Same `v`/`c` + factor/`opt` syntax as `transmission_limit`.",
    )
    interconnector: str = Field(
        "v1.0",
        description="Transmission expansion limit for lines/links with exactly one bus in `country` (interconnectors to/from `country`). Same `v`/`c` + factor/`opt` syntax as `transmission_limit`.",
    )
