...
def test_digrate_electricity():
    """"""
    import sys

    from esmtools.constants import BusCarrier, Carrier, DataModel, Group
    from esmtools.fileio import read_networks
    from esmtools.statistic import collect_myopic_statistics
    from esmtools.utils import filter_by, rename_aggregate

    sys.path.insert(0, r"C:\Storage\pypsa-eur-sec\scripts\Auswertung")

    from Toolbox.get_data_of_interest import get_time_series_production
    ...
    prod = get_time_series_production(networks[year], country, "AC")

    ac_supply = collect_myopic_statistics(
        networks,
        statistic="supply",
        comps=("StorageUnit", "Store", "Generator"),
        bus_carrier=BusCarrier.AC,
        aggregate_time=False,
    )  # .drop(Carrier.value_lost_load, level=DataModel.CARRIER, errors="ignore")
    ac_supply = filter_by(ac_supply, location=f"{country}0 0", year=year)

    # PHS exists in both, supply and withdrawal as "Storage Out"
    # and "Storage In", respectively. The old mappings for
    # e_production_ts and e_demand_ts only differ for PHS carrier.
    # By renaming PHS explicitly here to Storage Out, we can combine
    # the mappings and avoid some redundancy.
    ac_supply = rename_aggregate(ac_supply, {Carrier.phs: Group.storage_out})
    ...
    ac_production = filter_by(ac_production, location=f"{country}0 0", year=year)

    old_carrier = prod.columns
    new_carrier = ac_production.index.unique("carrier").union(
        ac_supply.index.unique("carrier")
    )

    old_carrier.difference(new_carrier)
    assert sorted(old_carrier) == sorted(new_carrier)

    assert prod.sum() == (ac_production.sum() + ac_supply.sum())
